...
//        }
        ...
        for(int i = 0; i < tableaux.getRowDimension() - 1; i++) {
            if(isSmaler[i]) {
                tableaux.set(i, i + table.getColumnDimension() - 1, 1.0); // tu
            }
            else {
                tableaux.set(i, i + table.getColumnDimension() - 1, -1.0);
            }
        }
        tableaux.set(tableaux.getRowDimension() - 1, tableaux.getColumnDimension() - 2, 1.0);
...
//                    tableaux.set(i, j, (-1*tableaux.get(pivotRow, j)) + tableaux.get(i, j));
    ...
    private void getSolution(Matrix tableaux) {
        for (int i = 0; i < tableaux.getColumnDimension() -1 - tableaux.getRowDimension(); i++) {
            ...
            for(int j = 0; j < tableaux.getRowDimension() -1 && b != -2; j++) {
